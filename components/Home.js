...
function Home() {
  ...
  useEffect(() => {
    fetch('https://mymoviz-backend-iw2w.vercel.app/movies')
  ...
  const movies = moviesData.map((data, i) => {
    ...
    const shortOverview = data.overview.length > 250 ? data.overview.slice(0,247) + '...' : data.overview;
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={shortOverview} poster={`https://image.tmdb.org/t/p/w500${data.poster_path}`} voteAverage={data.vote_average} voteCount={data.vote_count} />;
  });
